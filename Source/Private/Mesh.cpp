...
std::vector<Vertex> Mesh::CalculateNormals(std::vector<Vertex> vertices, std::vector<unsigned int> indices)
{
	for (unsigned int i = 0; i < indices.size(); i += 3) {
		...
		glm::vec3 vertex_1 = vertices[index_2].m_Position - vertices[index_1].m_Position;
		glm::vec3 vertex_2 = vertices[index_3].m_Position - vertices[index_1].m_Position;

		glm::vec3 normal = glm::normalize(glm::cross(vertex_1, vertex_2));

		vertices[index_1].m_Normal = (glm::normalize(vertices[index_1].m_Normal + normal));
		vertices[index_2].m_Normal = (glm::normalize(vertices[index_2].m_Normal + normal));
		vertices[index_3].m_Normal = (glm::normalize(vertices[index_3].m_Normal + normal));
	}
	return vertices;
}
